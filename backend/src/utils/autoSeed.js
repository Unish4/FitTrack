...
export const autoSeedPublicExercises = async () => {
  try {
    const existingCount = await Exercise.countDocuments({ isPublic: true });
    if (existingCount === 0) {
      console.log("No public exercises found in database. Starting automatic seed...");
      let seededCount = 0;
      for (const item of initialExercisesData) {
        await Exercise.create(item);
        seededCount++;
      }
      console.log(`✅ Auto-seeded ${seededCount} public exercises into database!`);
    } else {
      console.log(`ℹ️ Database already contains ${existingCount} public exercises.`);
    }
  } catch (error) {
    console.error("❌ Error auto-seeding public exercises:", error);
  }
};
